...
pub fn get_all_saves(app: AppHandle) -> Result<Vec<(String, String)>, UiError> {
    ...
    if installation_dir_path.exists() && installation_dir_path.is_dir() {
        for entry in std::fs::read_dir(installation_dir_path).map_err(|e| UiError::from(format!("Read dir error: {e}")))? {
            ...
            if path.is_dir() {
                ...
                if saves_path.exists() && saves_path.is_dir() {
                    for save_entry in std::fs::read_dir(saves_path).map_err(|e| UiError::from(format!("Read dir error: {e}")))? {
                        let save_entry = save_entry.map_err(|e| UiError::from(format!("Dir entry error: {e}")))?;
                                ...
                                if ext == "vcdbs" {
                                    if let Some(file_stem) = save_path.file_stem() {
                                        if let Some(save_name) = file_stem.to_str() {
                                            saves.push((save_name.to_string(), installation_name.clone()));
                                        }
                                    }
                                }
                            }
